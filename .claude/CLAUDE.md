...
9. **POLITIQUE DES LIENS — décision Andrianina, août 2026.** Trois obligations et une interdiction, dans cet ordre de priorité.
   - **DOFOLLOW PARTOUT.** Les jetons `rel` qui bloquent le suivi — `nofollow`, `ugc` **et `sponsored`** — sont retirés des `<a>` à deux endroits : à l'ingestion WordPress (`normalizeRelDofollowWp`, `proxy.js`) et à la publication (`stripFollowBlockers`, `src/utils/export.js`, appelée par `exportAsHtml` = passage obligatoire et unique de tout ce qui sort). `sponsored` est retiré **volontairement** : les liens externes de ces articles SONT les articles sponsorisés payants, et l'acheteur paie précisément pour du dofollow. Le risque de pénalité Google sur un lien payant non marqué a été énoncé et **assumé**. `noopener` / `noreferrer` sont **CONSERVÉS** : garde-fous navigateur, pas des directives de suivi (et les tests du verrou externe attendent `rel="noopener"` à l'identique).
